...
raw_welfare <- read.spss(file = "c:/R/Data/Koweps_hpc10_2015_beta1.sav",
                         to.data.frame = T, encoding= 'UTF-8')
...
# 지역 코드 목록 만들기
list_region <- data.frame(code_region = c(1,2,3,4,5,6,7), region = c("서울", "수도권(인천/경기)", "부산/경남/을산", "대구/경북", "대전,충남", "강원/충북", "광주/전남/전북/제주도"))
...
# 연령대 순으로 막대 색깔 나열하기
# 앞에서 만든 그래프는 막대 색깔이 young, old, middle 순으로 나열되어 있다.
# 이를 초년, 중년, 노년의 연령대 순으로 나열되도록 설정

# ageg 변수는 character 타입이기 때문에 levels가 없다.
class(region_ageg$ageg)

levels(region_ageg$ageg)

# factor()를 이용해 ageg 변수를 factor 타입으로 변환하고,
# level 파라미터를 이용해 순서를 지정한다.
...
# 그래프 막대 색깔이 연령대 순으로 나열되어 지역별 연령대 비율 구성을 쉽게 파악할 수 있음
ggplot(data = region_ageg, aes(x = region, y = pct, fill = ageg)) +
  geom_col() +
  coord_flip() +
  scale_x_discrete(limits = order)
